---
title: "Differential Expression Analysis- FACS EVT Analysis"
...
## Differential Expression Analysis- FACS EVT Analysis

Differential Expression analysis with batch correction using RUVr
...
```{r load data}
load(file="/active/paquette_a/slapehn/Placenta_CellLine_Comparison/Data/Merged Filtered Data/FilteredCounts_FACS_EVT_111723.RData")
Sample_Metadata <- read.csv("/active/paquette_a/slapehn/Placenta_CellLine_Comparison/SampleID_Key_FACS_EVTs_Nov2023.csv")
```
...
CellLine<-factor(Sample_Metadata$Cell_Line_Syn,levels=c("EVT", "Primary","BeWo", "BeWo_Syn", "JEG3", "HTR8SVneo", "JAR"))
...
#Step 2: Pull all results (order "EVT", "Primary","BeWo", "BeWo_Syn", "JEG3", "HTR8SVneo", "JAR")
...
### Plot DEG number
```{r plot DEGs}
Cell_Line_forDF <- c("Primary", "Primary", "BeWo", "BeWo", "BeWo Syn", "BeWo Syn", "JEG3", "JEG3", "HTR-8/SVneo", "HTR-8/SVneo", "JAR", "JAR")
...
DEG_number<- c(nrow(Sig_Primary_Increased), nrow(Sig_Primary_Decreased), nrow(Sig_BeWo_Increased), nrow(Sig_BeWo_Decreased), nrow(Sig_BeWo_Syn_Increased), nrow(Sig_BeWo_Syn_Decreased), nrow(Sig_JEG3_Increased), nrow(Sig_JEG3_Decreased), nrow(Sig_HTR8_Increased), nrow(Sig_HTR8_Decreased), nrow(Sig_JAR_Increased), nrow(Sig_JAR_Decreased))
...
plot_DF %>%
  ...
  ggtitle("Differentially Expressed Genes (FDR<0.01) compared to EVTs") + 
  scale_fill_manual(values=c("#0092B3", "#F09062")) + 
  ylim(0,6000)

#Plot DEG number without Directionality
DEG_number_nodirection <- c(nrow(Sig_Primary), nrow(Sig_BeWo), nrow(Sig_BeWo_Syn), nrow(Sig_JEG3), nrow(Sig_HTR8),nrow(Sig_JAR))

Cell_Line_forDF_nodirection <- c("Primary Trophoblasts", "BeWo", "BeWo Syncytialized", "JEG-3", "HTR-8/SVneo", "JAR")
...
plot_DF_nodirection %>%
  ...
  xlab("Placental Model") +
  ggtitle("Differentially Expressed Genes (FDR<0.01) compared to EVTs") + 
  scale_fill_manual(values=c("Primary Trophoblasts"="darkorange", "HTR-8/SVneo"="gold", "JAR"="green", "JEG-3"="purple", "BeWo"="red", "BeWo Syncytialized"="firebrick4")) +
  ylim(0,12000) +
  theme_minimal() +
...
color[which(Metadata_PCs_post$Cell_Line_Syn=="EVT")] = "blue"
...
Sig_Combined <- Sig_JEG3_BeWo_BeWoSyn_Primary_HTR8_JAR %>%
  dplyr::select(hgnc_symbol, logFC_JEG3, adj.P.Val_JEG3, logFC_BeWo, adj.P.Val_BeWo, logFC_BeWoSyn, adj.P.Val_BeWoSyn,logFC_Primary, adj.P.Val_Primary, logFC_HTR8, adj.P.Val_HTR8, logFC_JAR, adj.P.Val_JAR)
colnames(Sig_Combined) <- c("hgnc_symbol", "logFC JEG-3", "FDR JEG-3", "logFC BeWo", "FDR BeWo", "logFC BeWo Syncytialized", "FDR BeWo Syncytialized","logFC Primary Trophoblasts", "FDR Primary Trophoblasts", "logFC HTR-8/SVneo", "FDR HTR-8/SVneo", "logFC JAR", "FDR JAR")

#Export to be used as supplemental table
#write.csv(Sig_Combined, "/active/paquette_a/slapehn/Placenta_CellLine_Comparison/SupplementalMaterials/FACS_EVT_DEGs_SuppTable_120123.csv")
...
#Select columns of interest
AllResults_Combined <- AllResults_JEG3_BeWo_BeWoSyn_Primary_HTR8_JAR %>%
  dplyr::select(hgnc_symbol, logFC_JEG3, adj.P.Val_JEG3, logFC_BeWo, adj.P.Val_BeWo, logFC_BeWoSyn, adj.P.Val_BeWoSyn,logFC_Primary, adj.P.Val_Primary, logFC_HTR8, adj.P.Val_HTR8, logFC_JAR, adj.P.Val_JAR)
colnames(AllResults_Combined) <- c("hgnc_symbol", "logFC JEG-3", "FDR JEG-3", "logFC BeWo", "FDR BeWo", "logFC BeWo Syncytialized", "FDR BeWo Syncytialized","logFC Primary Trophoblasts", "FDR Primary Trophoblasts", "logFC HTR-8/SVneo", "FDR HTR-8/SVneo", "logFC JAR", "FDR JAR")

#Export to be used as supplemental table
#write.csv(AllResults_Combined, "/active/paquette_a/slapehn/Placenta_CellLine_Comparison/SupplementalMaterials/FACS_EVT_AllResults_SuppTable_120123.csv")
