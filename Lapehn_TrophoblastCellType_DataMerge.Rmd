---
title: "DataMerge- Trophoblast Cell Type Models"
...
## FACS Merge Sept 2024

This code is merging the unfiltered data from the Campbell et al. 2023 (Communications Biology) FAC Sorted/Vilous Digest placental tissue for CTBs, STBs, and EVTs with the cell line data. This merge will not include CANDLE/GAPPS placental tissue or villous explants since these are tissue based models rather than cell based models.
...
#Load Data
Data contains: 
FACS/Villous Digest STBs or CTBs or EVTs Samples
...
Primary_HTR8_Bewo_JEG3_JAR_TSC <- full_join(Primary_HTR8_Bewo_JEG3_JAR, TSC_All)
FullMerge_STB<- full_join(Primary_HTR8_Bewo_JEG3_JAR_TSC, STBs)
FullMerge_CTB<- full_join(Primary_HTR8_Bewo_JEG3_JAR_TSC, CTBs)
FullMerge_EVT<- full_join(Primary_HTR8_Bewo_JEG3_JAR_TSC, EVTs)
```
### Clean data names for STB
Need to make sure the Metadata is in the same order as the samples in the FullMerge dataset
```{r clean STB}
Sample_Metadata_STB <- read.csv("/active/paquette_a/slapehn/Placenta_CellLine_Comparison/SampleID_Key_FACS_STBs_Sept2024Revision.csv")
...
Need to make sure the Metadata is in the same order as the samples in the FullMerge dataset
```{r clean CTB}
Sample_Metadata_CTB <- read.csv("/active/paquette_a/slapehn/Placenta_CellLine_Comparison/SampleID_Key_FACS_CTBs_Sept2024Revision.csv")
...
Need to make sure the Metadata is in the same order as the samples in the FullMerge dataset
```{r clean EVT}
Sample_Metadata_EVT <- read.csv("/active/paquette_a/slapehn/Placenta_CellLine_Comparison/SampleID_Key_FACS_EVTs_Sept2024Revision.csv")
...
```{r save}
#save(FullMerge_NewIDs_Annotated_STB, file="/active/paquette_a/slapehn/Placenta_CellLine_Comparison/Data/Merged Unfiltered Data/FullMerge_Unfiltered_FACS_STB_091324.Rdata")

#save(FullMerge_NewIDs_Annotated_CTB, file="/active/paquette_a/slapehn/Placenta_CellLine_Comparison/Data/Merged Unfiltered Data/FullMerge_Unfiltered_FACS_CTB_091324.Rdata")

#save(FullMerge_NewIDs_Annotated_EVT, file="/active/paquette_a/slapehn/Placenta_CellLine_Comparison/Data/Merged Unfiltered Data/FullMerge_Unfiltered_FACS_EVT_091324.Rdata")
```