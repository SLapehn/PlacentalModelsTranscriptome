---
title: "Differential Expression Analysis- Placenta Tissue Model"
...
Differential Expression analysis with batch correction using RUVr and a significance threshold of FDR<0.01. Updating to include the trophoblast stem cells based on Placenta reviewer comments. 
...
```{r load data}
load(file="/active/paquette_a/slapehn/Placenta_CellLine_Comparison/Data/Merged Filtered Data/FilteredCounts_091124.RData")
Sample_Metadata <- read.csv("/active/paquette_a/slapehn/Placenta_CellLine_Comparison/SampleID_Key_Sept2024_Revision.csv")
```
...
CellLine<-factor(Sample_Metadata$Cell_Line,levels=c("PlacentaTissue", "Primary","BeWo", "JEG3", "HTR8SVneo", "JAR", "Explant", "TSC"))
...
### Plot DEG number
```{r plot DEGs}
Cell_Line_forDF <- c("Primary", "Primary", "BeWo", "BeWo", "JEG3", "JEG3", "HTR-8/SVneo", "HTR-8/SVneo", "JAR", "JAR", "Explant", "Explant", "hTSC", "hTSC")
...
DEG_number<- c(nrow(Sig_Primary_Increased), nrow(Sig_Primary_Decreased), nrow(Sig_BeWo_Increased), nrow(Sig_BeWo_Decreased), nrow(Sig_JEG3_Increased), nrow(Sig_JEG3_Decreased), nrow(Sig_HTR8_Increased), nrow(Sig_HTR8_Decreased), nrow(Sig_JAR_Increased), nrow(Sig_JAR_Decreased), nrow(Sig_Explant_Increased), nrow(Sig_Explant_Decreased),nrow(Sig_TSC_Increased), nrow(Sig_TSC_Decreased))
...
DEG_number_nodirection <- c(nrow(Sig_Primary), nrow(Sig_BeWo), nrow(Sig_JEG3), nrow(Sig_HTR8),nrow(Sig_JAR), nrow(Sig_Explant), nrow(Sig_TSC))

Cell_Line_forDF_nodirection <- c("Primary Trophoblasts", "BeWo", "JEG-3", "HTR-8/SVneo", "JAR", "Villous Explants", "hTSC")
...
plot_DF_nodirection %>%
  ...
  xlab("Placental Model") +
  ggtitle("Differentially Expressed Genes (FDR<0.01)") + 
  scale_fill_manual(values=c("Primary Trophoblasts"="darkorange", "HTR-8/SVneo"="gold", "Villous Explants"="magenta", "JAR"="green", "JEG-3"="purple", "BeWo"="red", "hTSC"="navy")) +
  ylim(0,12000) + 
  theme_minimal() +
...
hist(AllResults_TSC$logFC, xlim=c(-20,20), ylim=c(0,5000), breaks=20)
hist(Sig_TSC$logFC, xlim=c(-20,20), ylim=c(0,5000), breaks=20)
```
...
color[which(Metadata_PCs_post$Cell_Line=="TSC")] = "navy"
...
UnSig_Explant <- AllResults_Explant %>%
  dplyr::filter(adj.P.Val>0.01)
UnSig_TSC <- AllResults_TSC %>%
  dplyr::filter(adj.P.Val>0.01)

#Plot
Cell_Line_forUnSigDF <- c("Primary", "BeWo", "JEG3", "HTR-8/SVneo", "JAR", "Explant", "hTSC")

UnDEG_number<- c(nrow(UnSig_Primary), nrow(UnSig_BeWo), nrow(UnSig_JEG3), nrow(UnSig_HTR8), nrow(UnSig_JAR),  nrow(UnSig_Explant), nrow(UnSig_TSC))
...
Sig_TSC_PlacentaOnly <- inner_join(HPA_PlacentaOnly, Sig_TSC, by=c("Gene" ="hgnc_symbol"))
length(Sig_TSC_PlacentaOnly$Gene)
Sig_TSC_PlacentaEnriched <- inner_join(HPA_PlacentaEnriched, Sig_TSC, by=c("Gene" ="hgnc_symbol"))
length(Sig_TSC_PlacentaEnriched$Gene)
Sig_TSC_PlacentaEnhanced <- inner_join(HPA_PlacentaEnhanced, Sig_TSC, by=c("Gene" ="hgnc_symbol"))
length(Sig_TSC_PlacentaEnhanced$Gene)
```

## Plot HPA Overlap
``` {r HPA Plot DEGs}
Cell_Line_forDF <- c("Primary", "Primary", "Primary", "BeWo", "BeWo", "BeWo", "JEG3", "JEG3", "JEG3", "HTR-8/SVneo", "HTR-8/SVneo", "HTR-8/SVneo","JAR", "JAR", "JAR", "Explant", "Explant", "Explant", "TSC", "TSC", "TSC")
HPA_Category <- c("Placenta Only", "Placenta Enriched", "Placenta Enhanced", "Placenta Only", "Placenta Enriched", "Placenta Enhanced", "Placenta Only", "Placenta Enriched", "Placenta Enhanced", "Placenta Only", "Placenta Enriched", "Placenta Enhanced", "Placenta Only", "Placenta Enriched", "Placenta Enhanced", "Placenta Only", "Placenta Enriched", "Placenta Enhanced", "Placenta Only", "Placenta Enriched", "Placenta Enhanced")
Overlap_number<- c(length(Sig_Primary_PlacentaOnly$Gene), length(Sig_Primary_PlacentaEnriched$Gene), length(Sig_Primary_PlacentaEnhanced$Gene), length(Sig_BeWo_PlacentaOnly$Gene), length(Sig_BeWo_PlacentaEnriched$Gene), length(Sig_BeWo_PlacentaEnhanced$Gene), length(Sig_JEG3_PlacentaOnly$Gene), length(Sig_JEG3_PlacentaEnriched$Gene), length(Sig_JEG3_PlacentaEnhanced$Gene), length(Sig_HTR8_PlacentaOnly$Gene), length(Sig_HTR8_PlacentaEnriched$Gene), length(Sig_HTR8_PlacentaEnhanced$Gene), length(Sig_JAR_PlacentaOnly$Gene), length(Sig_JAR_PlacentaEnriched$Gene), length(Sig_JAR_PlacentaEnhanced$Gene), length(Sig_Explant_PlacentaOnly$Gene), length(Sig_Explant_PlacentaEnriched$Gene), length(Sig_Explant_PlacentaEnhanced$Gene), length(Sig_TSC_PlacentaOnly$Gene), length(Sig_TSC_PlacentaEnriched$Gene), length(Sig_TSC_PlacentaEnhanced$Gene))
...
Enhanced_TSC<- as.vector(Sig_TSC_PlacentaEnhanced$Gene)
...
Enriched_TSC<- as.vector(Sig_TSC_PlacentaEnriched$Gene)

#Step 2: Turn vectors into a list
Upset_list_sig_Enhanced<-list("Primary"=Enhanced_Primary, "BeWo"=Enhanced_BeWo, "HTR-8/SVneo"=Enhanced_HTR8, "JAR"=Enhanced_JAR, "JEG-3"=Enhanced_JEG3, "Villous Explants"=Enhanced_Explant, "TSC"=Enhanced_TSC)

Upset_list_sig_Enriched<-list("Primary"=Enriched_Primary, "BeWo"=Enriched_BeWo, "HTR-8/SVneo"=Enriched_HTR8, "JAR"=Enriched_JAR, "JEG-3"=Enriched_JEG3, "Villous Explants"=Enriched_Explant, "TSC"=Enriched_TSC)

#Step 3: Plot
upset_plot_Enhanced_Sig <- upset(fromList(Upset_list_sig_Enhanced), nsets=7,  text.scale=2, sets=c("Primary", "BeWo", "HTR-8/SVneo", "JAR", "JEG-3", "Villous Explants", "TSC"), keep.order=T)
upset_plot_Enhanced_Sig

upset_plot_Enriched_Sig <- upset(fromList(Upset_list_sig_Enriched), nsets=7,  text.scale=2, sets=c("Primary", "BeWo", "HTR-8/SVneo", "JAR", "JEG-3", "Villous Explants", "TSC"), keep.order=T)
...
UnSig_TSC$hgnc_symbol <- rownames(UnSig_TSC)
UnSig_TSC_PlacentaOnly <- inner_join(HPA_PlacentaOnly, UnSig_TSC, by=c("Gene" ="hgnc_symbol"))
length(UnSig_TSC_PlacentaOnly$Gene)
UnSig_TSC_PlacentaEnriched <- inner_join(HPA_PlacentaEnriched, UnSig_TSC, by=c("Gene" ="hgnc_symbol"))
length(UnSig_TSC_PlacentaEnriched$Gene)
UnSig_TSC_PlacentaEnhanced <- inner_join(HPA_PlacentaEnhanced, UnSig_TSC, by=c("Gene" ="hgnc_symbol"))
length(UnSig_TSC_PlacentaEnhanced$Gene)
```
...
UnSig_Enhanced_TSC<- as.vector(UnSig_TSC_PlacentaEnhanced$Gene)
...
UnSig_Enriched_TSC<- as.vector(UnSig_TSC_PlacentaEnriched$Gene)

#Step 2: Turn vectors into a list
Upset_list_Unsig_Enhanced<-list("Primary"=UnSig_Enhanced_Primary, "BeWo"=UnSig_Enhanced_BeWo, "HTR-8/SVneo"=UnSig_Enhanced_HTR8, "JAR"=UnSig_Enhanced_JAR, "JEG-3"=UnSig_Enhanced_JEG3, "Villous Explants"=UnSig_Enhanced_Explant, "TSC"=UnSig_Enhanced_TSC)

Upset_list_Unsig_Enriched<-list("Primary"=UnSig_Enriched_Primary, "BeWo"=UnSig_Enriched_BeWo, "HTR-8/SVneo"=UnSig_Enriched_HTR8, "JAR"=UnSig_Enriched_JAR, "JEG-3"=UnSig_Enriched_JEG3, "Villous Explants"=UnSig_Enriched_Explant, "TSC"=UnSig_Enhanced_TSC)

#Step 3: Plot
upset_plot_Enhanced <- upset(fromList(Upset_list_Unsig_Enhanced), nsets=7,  text.scale=2, sets=c("Primary", "BeWo", "HTR-8/SVneo", "JAR", "JEG-3", "Villous Explants", "TSC"), keep.order=T)
upset_plot_Enhanced

upset_plot_Enriched <- upset(fromList(Upset_list_Unsig_Enriched), nsets=7,  text.scale=2, sets=c("Primary", "BeWo", "HTR-8/SVneo", "JAR", "JEG-3", "Villous Explants", "TSC"), keep.order=T)
upset_plot_Enriched
```
...
TSC_forCombining <-Sig_TSC %>%
  dplyr::select(logFC, hgnc_symbol) %>%
  dplyr::rename(TSC=logFC)
...
DF5 <- full_join(DF4, Explant_forCombining, by=c("hgnc_symbol"))
DF_AllDEGs<- full_join(DF5, TSC_forCombining, by=c("hgnc_symbol"))
...
```{r}
VoomData <- v$E
#save(VoomData, file="/active/paquette_a/slapehn/Placenta_CellLine_Comparison/Data/VoomTransformed_091124.RData")
```
...
Sig_JEG3_BeWo_Primary_HTR8_JAR_Explant_TSC <- full_join(Sig_JEG3_BeWo_Primary_HTR8_JAR_Explant, Sig_TSC, by="hgnc_symbol")
Sig_Combined <- Sig_JEG3_BeWo_Primary_HTR8_JAR_Explant_TSC %>%
  dplyr::select(hgnc_symbol, logFC_JEG3, adj.P.Val_JEG3, logFC_BeWo, adj.P.Val_BeWo, logFC_Primary, adj.P.Val_Primary, logFC_HTR8, adj.P.Val_HTR8, logFC_JAR, adj.P.Val_JAR, logFC_Explant, adj.P.Val_Explant, logFC, adj.P.Val)
colnames(Sig_Combined) <- c("hgnc_symbol", "logFC JEG3", "FDR JEG3", "logFC BeWo", "FDR BeWo", "logFC Primary Trophoblasts", "FDR Primary Trophoblasts", "logFC HTR-8/SVneo", "FDR HTR-8/SVneo", "logFC JAR", "FDR JAR", "logFC Villous Explants", "FDR Villous Explants", "logFC TSC", "FDR TSC")

# Upload Cluster Membership of Genes
Clusters <- read.csv("/active/paquette_a/slapehn/Placenta_CellLine_Comparison/Results/Clustering Sept 2024/GeneClusterMembership_091124.csv")
...
ADMEClusters <- read.csv("/active/paquette_a/slapehn/Placenta_CellLine_Comparison/Results/Clustering Sept 2024/ADME_ClusterMembership_Annotation_091124.csv")
HPAClusters <- read.csv("/active/paquette_a/slapehn/Placenta_CellLine_Comparison/Results/Clustering Sept 2024/HPA_ClusterMembership_Annotation_091124.csv")

#combine with other dataframe
ADMEClusters <- ADMEClusters %>%
  dplyr::select(X, Annot_ADME)
...
#write.csv(Sig_Combined_wClusters_ADME_HPA, "/active/paquette_a/slapehn/Placenta_CellLine_Comparison/SupplementalMaterials_Sept2024/DEGs_Clusters_SupplementalTable_091124.csv")
...
AllResults_JEG3_BeWo_Primary_HTR8_JAR_Explant_TSC <- full_join(AllResults_JEG3_BeWo_Primary_HTR8_JAR_Explant, AllResults_TSC, by="hgnc_symbol")

AllResults_Combined <- AllResults_JEG3_BeWo_Primary_HTR8_JAR_Explant_TSC %>%
  dplyr::select(hgnc_symbol, logFC_JEG3, adj.P.Val_JEG3, logFC_BeWo, adj.P.Val_BeWo, logFC_Primary, adj.P.Val_Primary, logFC_HTR8, adj.P.Val_HTR8, logFC_JAR, adj.P.Val_JAR, logFC_Explant, adj.P.Val_Explant, logFC, adj.P.Val)
colnames(AllResults_Combined) <- c("hgnc_symbol", "logFC JEG-3", "FDR JEG-3", "logFC BeWo", "FDR BeWo", "logFC Primary Trophoblasts", "FDR Primary Trophoblasts", "logFC HTR-8/SVneo", "FDR HTR-8/SVneo", "logFC JAR", "FDR JAR", "logFC Villous Explants", "FDR Villous Explants", "logFC TSC", "FDR TSC")
...
#Export to be used as supplemental table
#write.csv(AllResults_Combined_wClusters_ADME_HPA, "/active/paquette_a/slapehn/Placenta_CellLine_Comparison/SupplementalMaterials_Sept2024/AllResults_Clusters_SupplementalTable_091124.csv")
...
#write.csv(Expr_forShiny, "/active/paquette_a/slapehn/Placenta_CellLine_Comparison/ForShinyApp/ExpressionData_091124.csv")
```
